...
Run: `go doc github.com/iurykrieger/lastro/internal/enums | grep -E "AngleBuild|KindAssertion|NatureComputational|OutputSingleShot"`

Expected: each name appears on a `const` line. The four constants used throughout this plan are: `enums.AngleBuild`, `enums.KindAssertion`, `enums.NatureComputational`, `enums.OutputSingleShot` (short-prefix names per E1's actual implementation in `internal/enums/enums.go` lines 12–128).
