...
import (
	...
	"example.com/http-api-sample/handlers"
)
...
func main() {
	...
	if sensorID == "" || useCaseID == "" || angle == "" || check == "" {
		fmt.Fprintf(os.Stderr, "usage: sensor-check --sensor=<id> --use-case=<id> --angle=<angle> <pass|bad-input>\n")
		os.Exit(2)
	}
	...
	case "pass":
		base.Verdict = "pass"
		base.Evidence = map[string]any{"expected": "trivial", "actual": "trivial"}
		emit(base)
	...
	case "bad-input":
		...
		if actual == 400 {
			base.Verdict = "pass"
			base.Evidence = map[string]any{"expected": 400, "actual": 400}
			emit(base)
		} else {
			base.Verdict = "fail"
			base.Evidence = map[string]any{"expected": 400, "actual": actual}
			base.HealHint = &healHint{
				Summary:   fmt.Sprintf("POST /orders with invalid body returned %d, expected 400", actual),
				Rationale: "Handler is missing the body-validation branch that returns 400 when required fields are absent.",
			}
			emit(base)
		}
